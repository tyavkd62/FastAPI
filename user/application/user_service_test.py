...
def user_service_dependencies(mocker):
    ...
    send_welcom_email_task_mock = mocker.Mock(spec=SendWelcomeEmailTask)
...
def test_create_user_success(user_service_dependencies):
    ...
    user_service = UserService(
        ...
        ulid=ulid_mock,
        crypto=crypto_mock,
        send_welcom_email_task=send_welcom_email_task_mock,
    )

    id = 'TEST_ID'
    name = 'John'
    email = 'tyavkd62@gmail.com'
    password = 'password'
    memo = 'Some memo'
    now = datetime.now()
    ...
    send_welcom_email_task_mock.delay.return_value = None
    send_welcom_email_task_mock.run.return_value = None
    ...
    send_welcom_email_task_mock.delay.assert_called_once_with(email)